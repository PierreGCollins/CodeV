...
void grnaxicon_v2(long* kerror, double* brind, double* coef, double* s, double* rindx, double* xngran)
{
	/* Local variables */
	...
	temp = 1 - pow(tempa,2)/2 + 5*pow(tempa,4)/24;
...
/* 	n*GRAD(n) */
	...
	xngran[1] = pow(*brind, 2) * coef[1] * s[1] * (5*pow(tempa,3)/6 - tempa)/rsq;
	xngran[2] = pow(*brind, 2) * coef[1] * s[2] * (5*pow(tempa,3)/6 - tempa)/rsq;
	xngran[3] = 0;
}
